...
def server(host="127.0.0.1", port=7000):
    ...
    while True:
        ...
        while True:
            ...
            if not data:
                break
            logging.info(f'[DEBUG] receiving data : {data}')
            #a = data.get('USERNAME')
            handle.save_file(data)
        sock.close()
        logging.info('[DEBUG] Sock Is Closing')
    listeningSock.close()
...
class NetAPI:
    ...
    def __init__(self, iHandle=None, oHandle=None, filelocation='file/tempfile'):
        ...
        otherweise Exceed Max File Size Limit
        '''
    ...
    def send_file(self,path, savingpath="file/Public"):
        ...
        try:
            ...
            #self.send_digest(md5)
            #self.send_tag(self.FILE_TARGET_TAG)
    ...
    def save_file(self,fileInformation):
        ...
        logging.info(f"[SHOW] FILEINFORMATION {fileInformation}")
        '''            
        ...
        if not content and not tempfile:
            ...
            hashkey = os.path.join(savepath, filename.split(".")[0]+"hash.txt")     
            if not os.path.exists(dirname):
                os.makedirs(dirname)
            ...
            # recv_block 
                ...
                #logging.info(f"tempfile is removed in {tempfile}")
